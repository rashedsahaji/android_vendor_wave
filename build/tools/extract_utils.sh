...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$WAVE_ROOT"/vendor/wave/build/tools/smali/baksmali.jar
        export SMALIJAR="$WAVE_ROOT"/vendor/wave/build/tools/smali/smali.jar
    fi
...
function extract() {
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [ "$SRC" = "adb" ]; then
            ...
            if [ -f "$SRC/$FILE" ]; then
                cp "$SRC/$FILE" "$DEST"
            # if file does not exist try WAVE target
